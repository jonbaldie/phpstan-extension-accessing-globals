...
class ForbidImpureGlobalFunctionsRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        $resolvedNameNode = new Name($resolvedFunctionName);

        if (!$this->reflectionProvider->hasFunction($resolvedNameNode, $scope)) {
            return [];
        }

        $function = $this->reflectionProvider->getFunction($resolvedNameNode, $scope);
